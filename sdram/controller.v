...
reg ReadSelected=0;  
reg [1:0] state=0;
always @(posedge clk) if(state==2'h0) ReadSelected <= read_now;
...
reg SDRAM_DQ_OE = 1'b0;
 always @(posedge clk) SDRAM_DQ_OE <= (state==2'h1) & WriteSelected;
reg [15:0] WrData1=0;
always @(posedge clk) WrData1 <= WrData;
reg [15:0] WrData2=0;
always @(posedge clk) WrData2 <= WrData1;
